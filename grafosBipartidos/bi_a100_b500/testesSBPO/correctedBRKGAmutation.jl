...
import Metaheuristics: environmental_selection!

# Inclui o arquivo com as funções de coloração gulosa
include("../../../colorGul.jl")

# variáveis globais (estruturas que serão reutilizadas muitas vezes)
global V = 0
global ADJ = Vector{Int}[]
global P_BUFFER = Int[]

function fitness_harmonious_coloring(individual::Vector{Float64})
    # sortperm! in-place usando o buffer global
...
# NOVO: Wrapper que aplica mutação em num_mutants indivíduos antes da seleção normal
struct MutateFirstReplacement <: Metaheuristics.AbstractEnvironmentalSelection
    num_mutants::Int
    base_selector::Metaheuristics.AbstractEnvironmentalSelection
end

function Metaheuristics.environmental_selection!(
    new_population,
    selection::MutateFirstReplacement,
    population,
    fitness,
    problem,
    options
)
    n_genes = length(population[1].x)
    
    # Primeiro: criar mutantes (cópias mutadas de indivíduos aleatórios)
    mutants = []
    for _ in 1:selection.num_mutants
        parent_idx = rand(1:length(population))
        mutant = copy(population[parent_idx])
        
        # Aplica GraphSwap mutation (mesmo código da mutation!)
        v1 = rand(1:n_genes)
        vizinhos = ADJ[v1]
        if !isempty(vizinhos)
            v2 = rand(vizinhos)
            mutant.x[v1], mutant.x[v2] = mutant.x[v2], mutant.x[v1]
        end
        push!(mutants, mutant)
    end
    
    # Adiciona mutantes à população atual
    extended_population = vcat(population, mutants)
    extended_fitness = vcat(fitness, [problem.f(x) for x in mutants])
    
    # Executa a seleção ambiental normal (ElitistReplacement)
    return Metaheuristics.environmental_selection!(
        new_population, 
        selection.base_selector, 
        extended_population, 
        extended_fitness, 
        problem, 
        options
...
# --- CONSTRUTOR DO ALGORITMO MODIFICADO ---
function MyStagnatedBRKGA(;
    ...
    # Cria o GA normal com ElitistReplacement
    base_ga = Metaheuristics.GA(;
        ...
        mutation = GraphSwapMutation(1.0), # Mantém igual
        environmental_selection = Metaheuristics.ElitistReplacement(), # Seleção normal
        kargs...
    )

    wrapped_params = StagnationWrapper(base_ga.parameters, stag_limit, 0, Inf)
    alg = Metaheuristics.Algorithm(wrapped_params, options = base_ga.options)
    
    # Substitui a seleção ambiental pelo wrapper que aplica mutação primeiro
    alg.parameters.internal_params.environmental_selection = MutateFirstReplacement(num_mutants, Metaheuristics.ElitistReplacement())
...
function run_brkga_experiment(k_limit::Int, N_pop::Int)
    ...
    # Define as proporções baseadas no N_pop total passado como argumento
    n_elite = 20
    n_mutant = 10
    n_offspring = N_pop - n_elite - n_mutant # Calcula offspring para manter tamanho da população

    # Instancia usando o construtor customizado
    alg = MyStagnatedBRKGA(
        num_elites = n_elite, 
...
function main()
    ...
    csv_path = "results_simples_BRKGAmutation_stag20_v3v4v5_progresso.csv"
