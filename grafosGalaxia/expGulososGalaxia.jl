...
# Geração da tabela de resumo (melhor resultado dentre as abordagens gulosas)
...
function generate_best_summary_table(results_data, filename, graph_type)
    ...
    # ajusta o título para refletir 'todos os v'
    title = "Melhor Resultado Global das Heurísticas Gulosas - Grafos $graph_type (a \\le 25, \\text{todos } v)"
...
# Geração da tabelha detahalda (resultados obtidos para cada guloso considerado)
...
function generate_detailed_table(results_data, filename, graph_type)
    ...
    # ajusta o título para refletir 'todos os v'
    title = "Resultados Detalhados das Heurísticas Gulosas - Grafos $graph_type (a \\le 25, \\text{todos } v)"
...
function main()
    ...
    println("--- Filtrando Arquivos Galaxy (a <= 25, incluindo TODOS os v) ---")
    for file_name in galaxy_files
        data = extract_galaxy_params(file_name)
        if data !== nothing
            a_param = data[:a_param]

            # CONSIDERAMOS SOMENTE AS GALAXIAS COM ATE 25 CENTROS, TALVEZ MUDAR PARA CONSIDERAR TODAS
            # lógica de filtragem
            if a_param <= 25 
                push!(filtered_file_names, file_name)
            else
                println("Pulando $file_name (a=$a_param > 25)")
            end
        else
    ...
    for (index, file_name) in enumerate(filtered_file_names)
        ...
        # armazena os resultados em uma lista de tuplas para facilitar a ordenação
        results_list = [
        ...
        # filtra resultados inválidos (onde a heurística falhou e retornou 0 cores)
        ...
        if isempty(valid_results)
            ...
            # melhor resultado é o primeiro da lista ordenada
        ...
        # Extrai N e M para armazenar nos resultados
        local N_v, N_e
        try
        ...
        # armazenamento para as Duas Tabelas
        
        # detalhada (resultado de cada uma das 3 heurísticas)
        ...
        # resumo (nova informação de resultado :best_heuristic)
    ...
    # geração dos arquivos Latex
    
    # tabela 1: Detalhada
    output_detailed = "results_Greedy_Galaxy_a01-a25_all_v_Detailed.tex"
    ...
    # tabela 2: Resumo
    output_summary = "results_Greedy_Galaxy_a01-a25_all_v_Summary.tex"
