...
# estruturas e métodos personalizados do GA
...
# ALTERAR PARA CROSSOVER QUE GERA SÓ 1 FILHO, SEM A RANDOMIZAÇÃO
...
function run_ga_experiment(file_name, k_limit, N_pop)
    ...
    function fitness_harmonious_coloring(individual::Vector{Float64})
        ...
        # Chamando a função de coloração gulosa com a lógica Harmônica
...
            se_colors,
            mean_time,
            se_time # ADD o erro padrão do tempo
        )
        
        body *= row
    end
...
function main()
    # encontra e filtra arquivos
    # MAIS UMA VEZ ESTAMOS CONSIDERANDO SOMENTE GALAXIAS COM ATE 25 CENTROS 
    # POSSIVELMENTE REMOVER O FILETRO PARA TESTAR TODAS AS QUANTIDADES POSSÍVEIS DE FILTROS
    
...
    # parâmetros do experimento (para cada arquivo/grafo considerado)
    N_REPETITIONS = 5
    K_LIMIT = 50      
    N_POP = 1000       
...
            # calcula métricas das 5 execuções
            mean_colors = mean(color_results)
            mean_time = mean(time_results)
            std_dev_colors = std(color_results)
            std_dev_time = std(time_results) # cálculo do Desvio Padrão do Tempo
            se_colors = std_dev_colors / sqrt(N_REPETITIONS)
            se_time = std_dev_time / sqrt(N_REPETITIONS) # cálculo do Erro Padrão do Tempo
            
            # armazena resultados
...
    # gera e salva a tabela de experimentos com o genético
