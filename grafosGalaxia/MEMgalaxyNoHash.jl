...
function main()
    # para os testes de memória usaremos somente os arquivos de versão 1
    all_files = filter(f -> startswith(f, "galaxy_") && endswith(f, "v1.col"), readdir())
    sort!(all_files)
    ...
    @showprogress 1 "Processando: " for (idx, file) in enumerate(all_files)
        ...
        for i in 1:n_rep
            ...
            # footprint de memória da run atual
            ...
            @printf("   Run %d Done (Chi: %d, Time: %.2fs, Mem: %.2f MB)\n", i, ch, elapsed, current_mem)
        end
        ...
        push!(results_main, Dict(
            ...
            :peak_mem_mb => maximum(mem_history), # pico máximo de memória
            :mean_mem_mb => mean(mem_history)     # média do uso de memória  
        ))
    end

    df = DataFrame(results_main)
    df = df[:, [:instancia, :a, :b, :c, :v, :N, :M, :mean_chi, :se_chi, :mean_time, :se_time, :peak_mem_mb, :mean_mem_mb]]

    CSV.write("results_GA_ADJ_MEM_noHash.csv", df)
    println("\nProcesso concluído. CSV gerado com sucesso.")
end
