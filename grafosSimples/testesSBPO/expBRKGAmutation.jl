...
function main()
    # filtros de instâncias a serem processadas
    FILTER_N = [100, 500, 1000, 1500, 2000] 
    FILTER_P = [1, 3, 5, 10, 20, 30, 40] 
    FILTER_V = [1, 2]

    # cofiguração dos testes
    N_REPETITIONS = 5
    K_STAG = 50
    N_POP = 100
    SAVE_EVERY = 10 # Flush de memória a cada 10 instâncias
    
    csv_path = "results_BRKGA_Mutation_progresso.csv"
    ...
    if isempty(all_files)
        println("AVISO: Nenhum arquivo corresponde aos filtros em $raiz_grafos.")
        return
    end

    sort!(all_files)
    println("BRKGA Custom Iniciado! Total de instâncias: ", length(all_files))
    ...
    @showprogress 1 "BRKGA + Mutation: " for (idx, file_path) in enumerate(all_files)
        ...
        # Carregamento e atualização de globais
        ...
        println("\n[$idx/$(length(all_files))] Processando: $file_name")
        ...
        # Flush e Append para liberar memória
        if idx % SAVE_EVERY == 0 || idx == length(all_files)
            CSV.write(csv_path, DataFrame(results_buffer), append=true)
    ...
    println("Arquivo gerado: $csv_path")
    println("="^40)
end
