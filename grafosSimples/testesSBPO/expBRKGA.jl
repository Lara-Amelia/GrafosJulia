...
function main()
    ...
    sort!(all_files)
    println("BRKGA Iniciado! Total de instâncias: ", length(all_files))

    # Preparação do CSV (Cabeçalho)
    header_df = DataFrame(
    ...
    @showprogress 1 "BRKGA Progress: " for (idx, file_path) in enumerate(all_files)
        ...
        println("\n[$idx/$(length(all_files))] BRKGA -> $file_name")

        t_tot, chi = Float64[], Int[]

        for i in 1:N_REPETITIONS
            elapsed = @elapsed begin
                # CORREÇÃO: Chama a função correta do BRKGA
    ...
    println("\n" * "="^40)
    println("EXPERIMENTOS BRKGA CONCLUÍDOS!")
    println("Resultados em: $csv_path")
    println("="^40)
end
